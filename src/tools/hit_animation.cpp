...
HitAnimation::HitAnimation (std::string atlas_path, std::string hurtbox_path, std::string hitbox_path)
    : Animation (atlas_path), hurtbox_path (hurtbox_path.c_str ()), hitbox_path (hitbox_path.c_str ())
{
...
{
    if (hitboxes.size () > 0)
        return hitboxes.at (frame % hitboxes.size ());
    else
...
{
    if (hurtboxes.size () > 0)
        return hurtboxes.at (frame % hurtboxes.size ());
    else
        return std::vector<SDL_Rect> {};
}
